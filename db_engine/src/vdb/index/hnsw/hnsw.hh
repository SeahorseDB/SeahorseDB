...
#include "vdb/index/hnsw/common/visited_list_pool.hh"
...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
  ...
  static constexpr tableint INVALID_NODE = static_cast<tableint>(-1);

  size_t max_elements_{0};
  mutable std::atomic<size_t> cur_element_count{
      0};  // current number of elements
  mutable std::atomic<size_t> complete_element_count{
      0};  // complete number of elements
  ...
  HierarchicalNSW(vdb::DistanceSpace space, size_t dim,
                  const std::string &location,
                  [[maybe_unused]] bool nmslib = false, size_t max_elements = 0,
                  bool allow_replace_deleted = false)
      : allow_replace_deleted_(allow_replace_deleted) {
    loadIndex(location, space, dim, max_elements);
  }
  ...
  HierarchicalNSW(vdb::DistanceSpace space, size_t dim, size_t max_elements,
    ...
    if (M <= 10000) {
      ...
      std::cerr << "warning: M parameter exceeds 10000 which may lead to "
                   "adverse effects."
                << std::endl;
      std::cerr << "         Cap to 10000 will be applied for the rest of the "
                   "processing."
                << std::endl;
      M_ = 10000;
    }
    ...
    ARROW_CAST_OR_NULL(
        data_level0_memory_, char *,
        AllocateNoPrefix(max_elements_ * size_data_per_element_));
    if (data_level0_memory_ == nullptr)
      throw std::runtime_error("Not enough memory");

    cur_element_count = 0;
    complete_element_count = 0;
    ...
    enterpoint_node_ = -1;
    maxlevel_ = -1;
    ARROW_CAST_OR_NULL(linkLists_, char **,
                       AllocateNoPrefix(sizeof(void *) * max_elements_));
  ...
  void clear() {
    if (data_level0_memory_) {
      DeallocateNoPrefix(data_level0_memory_,
    ...
    if (linkLists_) {
      for (tableint i = 0; i < cur_element_count; i++) {
        if (element_levels_[i] > 0)
          DeallocateNoPrefix(linkLists_[i],
                             size_links_per_element_ * element_levels_[i] + 1);
      }
      DeallocateNoPrefix(linkLists_, sizeof(void *) * max_elements_);
      linkLists_ = nullptr;
    }
    cur_element_count = 0;
  ...
  size_t getCompleteElementCount() { return complete_element_count; }

  size_t getDeletedCount() { return num_deleted_; }

  DISTFUNC_FLOAT32 getDistFunc() { return fstdistfunc_; }

  void *getDistFuncParam() { return dist_func_param_; }
...
//                    if (candidate_id == 0) continue;
  ...
  searchBaseLayerST(
    ...
    uint64_t calculation_count = 0;
    uint64_t candidate_update_count = 0;
...
//                    if (candidate_id == 0) continue;
    ...
    vdb::metrics::CollectValue(
        vdb::metrics::MetricIndex::HnswSearchKnnComputationCountPerQuery,
        calculation_count);
    vdb::metrics::CollectValue(
        vdb::metrics::MetricIndex::HnswSearchKnnCandidateUpdateCountPerQuery,
        candidate_update_count);
    vdb::metrics::CollectValue(
        vdb::metrics::MetricIndex::HnswSearchKnnHopCountPerQuery, hop_count);
    visited_list_pool_->releaseVisitedList(vl);
    return top_candidates;
  }

  // This function is multi-threaded version of searchBaseLayerST
  template <bool bare_bone_search = true, bool collect_metrics = false>
  std::priority_queue<std::pair<dist_t, tableint>,
                      std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
  searchBaseLayerST_MT(
      tableint ep_id, const void *data_point, size_t ef, size_t nthreads,
      BaseFilterFunctor *isIdAllowed = nullptr,
      BaseSearchStopCondition<dist_t> *stop_condition = nullptr) const {
    VisitedList *vl = visited_list_pool_->getFreeVisitedList();
    vl_type *visited_array = vl->mass;
    ...
    std::mutex candidate_set_mtx, top_candidates_mtx;
    ...
    std::atomic<bool> starting_flag = false;

    for (size_t i = 0; i < nthreads; i++) {
      threads.emplace_back(
          [&](int tid) {
            while (!starting_flag && tid != 0) std::this_thread::yield();
            while (!candidate_set.empty()) {
              std::pair<dist_t, tableint> current_node_pair;
              {
                std::lock_guard<std::mutex> lock(candidate_set_mtx);
            ...
            //                    if (candidate_id == 0) continue;
                ...
                if (!(visited_array[candidate_id] == visited_array_tag)) {
                  ...
                  if (!bare_bone_search && stop_condition) {
                    flag_consider_candidate =
                        stop_condition->should_consider_candidate(dist,
                                                                  lowerBound);
                  } else {
                    flag_consider_candidate =
                        top_candidates.size() < ef || lowerBound > dist;
                  ...
                  if (flag_consider_candidate) {
                    ...
                    if (bare_bone_search ||
                        (!isMarkedDeleted(candidate_id) &&
                         ((!isIdAllowed) ||
                          (*isIdAllowed)(getExternalLabel(candidate_id))))) {
                      std::lock_guard<std::mutex> lock(top_candidates_mtx);
                      top_candidates.emplace(dist, candidate_id);
                      if (!bare_bone_search && stop_condition) {
                        stop_condition->add_point_to_result(
                            getExternalLabel(candidate_id), currObj1, dist);
...

                    {
                      ...
                      while (flag_remove_extra) {
                        ...
                        if (!bare_bone_search && stop_condition) {
                          stop_condition->remove_point_from_result(
                              getExternalLabel(id), getDataByInternalId(id),
                              dist);
                          flag_remove_extra =
  ...
  tableint mutuallyConnectNewElement(
      [[maybe_unused]] const void *data_point, tableint cur_c,
      std::priority_queue<std::pair<dist_t, tableint>,
                          std::vector<std::pair<dist_t, tableint>>,
                          CompareByFirst> &top_candidates,
  ...
  void saveIndex(const std::string &location) {
    std::ofstream output = std::ofstream(location, std::ios::binary);
  ...
  void loadIndex(const std::string &location, vdb::DistanceSpace space,
                 size_t dim, size_t max_elements_i = 0) {
    std::ifstream input;
    ...
    complete_element_count = cur_element_count.load();
    ...
    ARROW_CAST_OR_NULL(data_level0_memory_, char *,
                       AllocateNoPrefix(max_elements * size_data_per_element_));
    if (data_level0_memory_ == nullptr)
      throw std::runtime_error(
    ...
    ARROW_CAST_OR_NULL(linkLists_, char **,
                       AllocateNoPrefix(sizeof(void *) * max_elements));
    if (linkLists_ == nullptr)
      throw std::runtime_error(
    ...
    for (size_t i = 0; i < cur_element_count; i++) {
      ...
      if (linkListSize == 0) {
        ...
        ARROW_CAST_OR_NULL(linkLists_[i], char *,
                           AllocateNoPrefix(linkListSize + 1));
  ...
  tableint addPoint(const void *data_point, labeltype label, int level) {
    ...
    if (curlevel) {
      ARROW_CAST_OR_NULL(
          linkLists_[cur_c], char *,
          AllocateNoPrefix(size_links_per_element_ * curlevel + 1));
      ...
      if (curlevel < maxlevelcopy) {
        ...
        for (int level = maxlevelcopy; level > curlevel; level--) {
          ...
          while (changed) {
            ...
            for (int i = 0; i < size; i++) {
              tableint cand = datal[i];
              if (cand > max_elements_) throw std::runtime_error("cand error");
  ...
  std::priority_queue<std::pair<dist_t, labeltype>> searchKnn(
    ...
    if (enterpoint_node_ == INVALID_NODE) return result;
  ...
  std::priority_queue<std::pair<dist_t, labeltype>> searchKnn_MT(
      const void *query_data, size_t k, size_t nthreads,
    ...
    bool bare_bone_search = !num_deleted_ && !isIdAllowed;
    if (bare_bone_search) {
      top_candidates = searchBaseLayerST_MT<true>(
          currObj, query_data, std::max(ef_, k), nthreads, isIdAllowed);
    } else {
      top_candidates = searchBaseLayerST_MT<false>(
          currObj, query_data, std::max(ef_, k), nthreads, isIdAllowed);
    }
